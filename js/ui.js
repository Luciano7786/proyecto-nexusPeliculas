...
export const renderizarPeliculas = (peliculas, contenedor, generos = []) => {
    ...
    peliculasValidas.forEach((pelicula, indice) => {
        ...
        tarjeta.innerHTML = `
            <span class="fav-heart ${claseFav}" data-id="${pelicula.id}">♥</span>
            <span class="watchlist-btn ${clasePendiente}" data-id="${pelicula.id}">🕒</span>
            <img src="${rutaPoster}" alt="${pelicula.title}" class="movie-poster" loading="lazy">
            <div class="movie-info">
                <h3 class="movie-title">${pelicula.title}</h3>
                <span class="movie-rating">★ ${pelicula.vote_average.toFixed(1)}</span>
            </div>
        `;
...
export const renderizarDetallesPelicula = (pelicula, contenedor, claveVideo = null, creditos = null, recomendaciones = null, resenas = null) => {
            ...
            htmlReparto = `
                <div class="cast-section">
                    ...
                    <div class="cast-container">
                        ${repartoValido.slice(0, 10).map(actor => `
                            <div class="cast-card">
                                <img src="${CONFIGURACION.URL_BASE_IMAGEN + actor.profile_path}" alt="${actor.name}">
                                <p>${actor.name}</p>
    ...
    if (recomendaciones && recomendaciones.results) {
        const recomendacionesValidas = recomendaciones.results.filter(rec => rec.poster_path);
        if (recomendacionesValidas.length > 0) {
            htmlRecomendaciones = `
                <div class="recommendations-section">
                    <h3>Recomendaciones</h3>
                    <div class="recommendations-container">
                        ${recomendacionesValidas.slice(0, 5).map(rec => `
                            <div class="rec-card" data-id="${rec.id}">
                                <img src="${CONFIGURACION.URL_BASE_IMAGEN + rec.poster_path}" alt="${rec.title}" title="${rec.title}">
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    if (resenas && resenas.results && resenas.results.length > 0) {
        htmlResenas = `
            <div class="reviews-section">
                ...
                ${resenas.results.slice(0, 3).map(resena => `
                    <div class="review-card">
                        <div class="review-header">
                            <span>${resena.author}</span>
                            <span>★ ${resena.author_details.rating || '-'}</span>
                        </div>
        ...
        htmlResenas = '<div class="reviews-section"><p>No hay reseñas disponibles.</p></div>';
    }

    contenedor.innerHTML = `
        <div class="movie-details-content" style="display: flex; gap: 2rem; flex-wrap: wrap;">
            <img src="${rutaPoster}" alt="${pelicula.title}" style="width: 300px; border-radius: 8px;" loading="lazy">
            <div style="flex: 1;">
                ...
                <p style="margin-top: 1rem;"><strong>Calificación:</strong> ★ ${pelicula.vote_average.toFixed(1)}</p>
                ${htmlVideo}
                ${htmlReparto}
                ${htmlRecomendaciones}
                ${htmlResenas}
